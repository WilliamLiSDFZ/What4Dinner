...
- **Layout** — `components/Layout.jsx` renders the fixed sidebar (brand, `navItems` as `NavLink`s, user chip) and the scrollable `<main>`. A floating "Add new dish" FAB is rendered globally except on the `/add` route.
